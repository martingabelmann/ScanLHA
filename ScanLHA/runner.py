...
class BaseRunner():
    ...
    def runBinary(self, *args, cwd = None): # noqa: E901
...
class MicrOmegas(SLHARunner):
    def __init__(self,conf):
        ...
        logging.info('running "make clean" on MicrOmegas installtion.')
        Popen('yes|make clean', shell=True, stdout=DEVNULL, stderr=DEVNULL, cwd=self.omegadir)
        logging.info('running "make" on MicrOmegas installtion.')
        i = 0
        while not os.path.isfile(self.omegadir + '/CalcHEP_src/FlagsForMake') and i < 5:
        ...
        logging.info('running "make clean" on MicrOmegas model.')
        os.chdir(self.modeldir)
        Popen(['make', 'clean'], stdout=DEVNULL, stderr=DEVNULL, shell=True, cwd=self.modeldir)
        logging.info('running "make main={}" on MicrOmegas model.'.format(self.config['omegamain']))
        omegabin = self.config['omegamain'].replace('.cpp','')
        i = 0
        while not os.path.isfile(omegabin) and i < 5:
    ...
    def execute(self, params):
        ...
        slha = self.read(fout) # need to read twice to check higgs mass constraint befor MicrOmegas
        if 'log' in slha:
            slha.pop('log')
